...
import {
  CustomerGrowthQueryDto,
  TrendPeriod,
} from '../Analytics/dto/customer-growth-query.dto';
...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    try {
      const result = await this.userRepo.manager.transaction(
        async (manager) => {
          ...
          const oRepo = manager.getRepository(
            this.otpRepo.metadata.target as any,
          ); // or manager.getRepository(OtpEntity)
          ...
          let customerRole = await rRepo.findOne({
            where: { name: 'customer' },
          });
          if (!customerRole) {
      ...
      // unknown error
      this.logger.error(
        'Failed to register user (unexpected error)',
    ...
    try {
      ...
      this.logger?.warn(
        'Failed to emit identity.user.registered event',
        err as any,
      );
    }
  ...
  async login(user: User, deviceInfo: string, ip: string, remember = false) {
    ...
    try {
      ...
      if (!restaurantId) {
        this.logger.debug(
          `User ${user.user_id} has role restaurant_owner but no restaurants found`,
        );
        // }
      }
    } catch (err) {
      this.logger.warn(
        'Failed to fetch restaurant_id for restaurant_owner',
        err as any,
      );
    ...
    const refreshDays = remember
      ? Number(process.env.JWT_REMEMBER_DAYS ?? 30)
      : Number(process.env.JWT_REFRESH_DAYS ?? 7);
  ...
  async refresh(dto: RefreshTokenDto) {
    ...
    if (stored.revoked) {
      ...
      await this.tokenRepo.update(
        { user: { user_id: stored.user.user_id } },
        { revoked: true },
      );
    ...
    if (!valid) {
      ...
      throw new UnauthorizedException(
        'Invalid refresh token (possible theft). All sessions revoked.',
      );
    }
  ...
  private async createAndSendOtp(
    ...
    try {
      ...
      this.logger.warn(
        'Failed to send OTP email in createAndSendOtp',
        err as any,
      );
    }
  ...
  async resendOtp(
    ...
    if (last && !last.used && last.expires_at > new Date()) {
      ...
      return {
        ok: false,
        message: `OTP still valid. Try again in ${remainingMin} minute(s)`,
      };
    }
  ...
  async resetPassword(dto: ResetPasswordDto) {
    ...
    // emit kafka / audit
    try {
      await this.kafka.emit('identity.user.password_reset', {
        user_id: user.user_id,
        at: new Date(),
      });
    } catch (err) {
  ...
  async deleteUserIfCustomer(
    ...
    const hasCustomerRole =
      ...
      user.roles.some((ur) => ur.role && ur.role.name === 'customer');
    if (!hasCustomerRole)
      throw new ForbiddenException(
        'Only users with the customer role can be deleted',
      );
  ...
  async deleteAllCustomers(): Promise<{ deleted: number; message: string }> {
    ...
    if (userIds.length === 0)
      return { deleted: 0, message: 'no_customers_found' };

    try {
      const deleteResult = await this.userRepo.manager.transaction(
        async (manager) => {
      ...
      throw new InternalServerErrorException(
        'Failed to delete customers. Please try again later.',
      );
    }
  }

  async getCustomerGrowth(
    query: CustomerGrowthQueryDto,
  ): Promise<CustomerGrowthDto[]> {
    this.logger.log(
      `Fetching customer growth data for period: ${query.period}`,
    );

    const days =
      query.period === TrendPeriod.WEEK
        ? 7
        : query.period === TrendPeriod.QUARTER
          ? 90
          : 30;
